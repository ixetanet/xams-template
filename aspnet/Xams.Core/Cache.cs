...
namespace Xams.Core
{
    public class Cache
    {
        ...
        internal static async Task Initialize(IDataService dataService)
        {
            ...
            dbContext.SetAuditEnabled(false);
            ...
            foreach (var assembly in assemblies)
            {
                foreach (var type in assembly.GetTypes())
                {
                    ...
                    ServiceHubInfo serviceHubInfo = new()
                    {
                        ServiceHubAttribute = serviceHubAttribute,
                        Type = type
                    };

                    if (cache.ServiceHubs.ContainsKey(serviceHubAttribute.Name))
                    {
                        throw new Exception($"Duplicate Hubs with Name: {serviceHubAttribute.Name}");
                    }

                    cache.ServiceHubs.Add(serviceHubAttribute.Name, serviceHubInfo);
                }
            }
        ...
        public class ServiceHubInfo
        {
            public ServiceHubAttribute ServiceHubAttribute { get; internal set; } = null!;
            public Type Type { get; internal set; } = null!;
        }
