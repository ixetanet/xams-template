...
namespace Xams.Core.Services
{
    ...
    public class DataService<TDbContext> : DataService<TDbContext, User>
        ...
        new()
    {
        public DataService(ILogger<DataService<TDbContext, User, Team, Role, Setting>> logger,
    ...
    public class DataService<TDbContext, TUser> : DataService<TDbContext, TUser, Team>
        ...
        new()
    {
        public DataService(ILogger<DataService<TDbContext, TUser, Team, Role, Setting>> logger,
            IServiceProvider serviceProvider) : base(logger, serviceProvider)
        {
        }
    }
    ...
    public class DataService<TDbContext, TUser, TTeam> : DataService<TDbContext, TUser, TTeam, Role>
        ...
        new()
    {
        public DataService(ILogger<DataService<TDbContext, TUser, TTeam, Role, Setting>> logger,
            IServiceProvider serviceProvider) : base(logger, serviceProvider)
        {
        }
    }
    ...
    public class DataService<TDbContext, TUser, TTeam, TRole> : DataService<TDbContext, TUser, TTeam, TRole, Setting>
        ...
        new()
    {
        public DataService(ILogger<DataService<TDbContext, TUser, TTeam, TRole, Setting>> logger,
            IServiceProvider serviceProvider) : base(logger, serviceProvider)
        {
        }
    }

    public class DataService<TDbContext, TUser, TTeam, TRole, TSetting> : IDataService, IDisposable
        ...
        new()
    {
            ...
            IServiceProvider serviceProvider)
        {
            ...
            _dataRepository = new DataRepository(typeof(TDbContext), this);
        }
        ...
        public async Task<Response<ReadOutput>> Read(Guid userId, ReadInput readInput, PipelineContext? parent = null)
        {
            ...
            {
                Logger.LogError(e, "ExecutionId {executionId} Read Failed UserId {userId}: {Message}", _executionId,
                    userId, e.Message);
                return new Response<ReadOutput>()
                {
                    Succeeded = false,
        ...
        public async Task<Response<object?>> Action(Guid userId, ActionInput input, HttpContext httpContext)
        {
            ...
            var response = await ExecuteTransaction(userId, async (pipelineContext) =>
            {
                pipelineContext.File = file;
                pipelineContext.InputParameters = input.parameters ?? new Dictionary<string, JsonElement>();
        ...
        public async Task<Response<object?>> ExecuteTransaction(Guid userId, Func<PipelineContext,Task<Response<object?>>> action)
        {
            var pipelineContext = new PipelineContext()
            {
                UserId = userId,
            ...
            {
                Logger.LogError(e, "ExecutionId {executionId} Action Failed UserId {userId}: {Message}", _executionId,
                    userId, e.Message);
                await _dataRepository.RollbackTransaction();
                return new Response<object?>()
                {
        ...
        private async Task<Response<object?>> BulkExecute(Guid userId, BulkInput input, bool returnArray)
        {
            try
            {
                ...
                operations.AddRange(creates.Select(x => new OperationInfo()
                {
                    ...
                    Entity = EntityUtil.GetEntity(x, DataOperation.Create, out string error).Data ??
                ...
                operations.AddRange(updates.Select(x => new OperationInfo()
                {
                    TableName = x.tableName ?? "",
                    DataOperation = DataOperation.Update,
                    Entity = EntityUtil.GetEntity(x, DataOperation.Update, out string error).Data ??
                             throw new Exception(error),
                    Input = x
                }));
                ...
                operations.AddRange(deletes.Select(x => new OperationInfo()
                {
                    ...
                    Entity = EntityUtil.GetEntity(x, DataOperation.Delete, out string error).Data ??
                             throw new Exception(error),
                    Input = x
                }));
            ...
            {
                Logger.LogError(ex, "ExecutionId {executionId} Operation Failed UserId {userId}: {Message}",
                    _executionId, userId, ex.Message);
                await _dataRepository.RollbackTransaction();
                throw;
            }
            finally
            ...
            object? parameters = null, PipelineContext? parent = null)
        {
            ...
            var preEntity = dataOperation is DataOperation.Delete or DataOperation.Update
            ...
            BulkStage bulkStage, Guid userId)
        {
            ...
            foreach (var preBulkServiceLogic in preBulkServiceLogics)
            {
                ...
                {
                    Logger.LogError(e, "ExecutionId {executionId} Bulk ServiceLogic Failed UserId {userId}: {Message}",
                        _executionId, userId, e.Message);
            ...
            bool checkSecurity = true)
        {
            ...
            var tableGroups = (from pipelineContext in updateDeletePContexts
            ...
            try
            {
                await Parallel.ForEachAsync(tableGroups, cts.Token, async (tableGroup, _) =>
                {
                    ...
                    foreach (var pipelineContext in tableGroup.PipelineContexts.Where(x => x.PreEntity == null))
                    {
                        ...
                        var id = pipelineContext.Entity?.GetValue<dynamic>(metadata.PrimaryKey);
                        var idType = id?.GetType();
                        if (id == null || (idType == typeof(Guid) && id == Guid.Empty) || (idType != typeof(Guid) && id == 0))
                        {
                            cancelledResponse = new Response<object?>()
                            {
                                FriendlyMessage = "Update or Delete operation requires the primary key.",
                                Succeeded = false
                            };
                        ...
                        if (!preEntities.ContainsKey(id))
                        {
                            cancelledResponse = new Response<object?>()
                            {
                                Succeeded = false,
                                FriendlyMessage =
                                    $"Entity with Id {id} not found in {pipelineContext.TableName} table.",
                            };
                            await cts.CancelAsync();
                            return;
                        }

                        pipelineContext.PreEntity = preEntities[id];
                        
                        // Check security is currently only false for deletes
                        if (!checkSecurity)
                        {
                            continue;
                        }
