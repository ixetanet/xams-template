...
public class BaseServiceContext(PipelineContext pipelineContext)
{
    ...
    public async Task<Response<object?>> HubSend<T>(object message) where T : class, IServiceHub
    {
        ...
        var hubInstance = ActivatorUtilities.CreateInstance(DataService.ServiceProvider, hubType);
            
        if (hubInstance == null)
        {
            throw new Exception($"Failed to create instance of hub {hubType.Name}");
        }
        
        var signalRInstance = DataService.ServiceProvider.GetRequiredService<IHubContext<SignalRHub>>();
        var hub = (IServiceHub)hubInstance;
        try
        {
            var scope = DataService.ServiceProvider.CreateScope();
        ...
        {
            Logger.LogError($"Failed to send message to hub {hubType.Name}: {e}");
            return new Response<object?>()
            {
                Succeeded = false,
                LogMessage = $"Failed to send message to hub {hubType.Name}: {e.Message}",
            };
        }
    }
